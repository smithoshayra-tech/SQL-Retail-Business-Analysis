...
-- 2c. Where Tennessee stores sit nationally
SELECT state, Avg_ListPrice, DENSE_RANK() OVER (ORDER BY Avg_ListPrice DESC) AS PriceRank
FROM (
  SELECT a.State, AVG(p.ListPrice) AS Avg_ListPrice
  FROM Purchase p
  JOIN Store s ON p.StoreID = s.StoreID
  JOIN Address a ON s.AddressID = a.AddressID
  GROUP BY a.State
) AS state_avg;
-- Tennessee stores are ranked at the 34th position nationally
